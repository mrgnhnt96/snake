...
import 'package:nocterm_nested/nested.dart';
...
class MultiProvider extends Nested {
  ...
  static List<SingleChildComponent> _collapseProviders(
    ...
    Component Function(Component? child)? previous;
    ...
    return [
      if (previous != null)
        SingleChildBuilder(builder: (context, child) => previous!(child)),
...
class Provider<T> extends InheritedProvider<T> {
  ...
  static T of<T>(BuildContext context, {bool listen = true}) {
    assert(listen == false || debugIsInInheritedProviderUpdate, '''
Tried to listen to a value exposed with provider, from outside of the Component tree.
...
It is unsupported because may pointlessly rebuild the Component associated to the
event handler, when the Component tree doesn't care about the value.
...
extension ReadContext on BuildContext {
  ...
  /// It will _not_ make Component rebuild when the value changes and should not be
